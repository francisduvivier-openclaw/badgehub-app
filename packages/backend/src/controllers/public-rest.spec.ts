...
async function post(app: App, url: string) {
...
describe(
  ...
  () => {
    ...
    test("GET /api/v3/badges", async () => {
      const res = await get(app, "/api/v3/badges");
      expect(res.statusCode).toBe(200);
      expect(res.body).toContain("why2025");
    });

    test("GET /api/v3/project-summaries", async () => {
      const res = await get(app, "/api/v3/project-summaries");
      expect(res.statusCode).toBe(200);
      expect(
    ...
    test("GET /api/v3/project-summaries should allow sorting by installs", async () => {
      const res = await get(app, "/api/v3/project-summaries?orderBy=installs");
      expect(res.statusCode).toBe(200);
    ...
    test("reporting an install should update installs in project summaries", async () => {
      ...
      const baselineRes = await get(app, "/api/v3/project-summaries");
      ...
      const reportRes = await post(app, `/api/v3/projects/${projectSlug}/rev1/report/install?id=${randomUUID()}`);
      expect(reportRes.statusCode).toBe(204);

      await createBadgeHubData().refreshReports();

      const updatedRes = await get(app, "/api/v3/project-summaries");
    ...
    test("GET /api/v3/project-summaries should not contain hidden apps unless the slug is given", async () => {
      const res = await get(app, "/api/v3/project-summaries");
      expect(res.statusCode).toBe(200);
      expect(res.body.find((app: ProjectSummary) => app.slug === "nanogames")).toBeUndefined();
      expect(res.body.find((app: ProjectSummary) => app.hidden)).toBeUndefined();

      const withSlugRes = await get(app, "/api/v3/project-summaries?slugs=nanogames");
      expect(withSlugRes.statusCode).toBe(200);
      expect(
        withSlugRes.body.find((app: ProjectSummary) => app.slug === "nanogames")?.hidden
      ).toBe(true);
    });

    test("GET /api/v3/project-summaries with device filter", async () => {
      const res = await get(app, "/api/v3/project-summaries?badge=why2025");
      expect(res.statusCode).toBe(200);
      expect(res.body.every((app: ProjectSummary) => app.badges?.includes("why2025"))).toBe(true);
      expect(res.body.find((app: ProjectSummary) => app.slug === "codecraft")).toBeDefined();
    });

    test("GET /api/v3/project-summaries with category filter", async () => {
      const res = await get(app, "/api/v3/project-summaries?category=Silly");
      expect(res.statusCode).toBe(200);
      expect(res.body.length).toMatchInlineSnapshot(`4`);
      expect(res.body.every((app: ProjectSummary) => app.categories?.includes("Silly"))).toBe(true);
      expect(res.body.find((app: ProjectSummary) => app.categories?.includes("Silly"))).toBeDefined();
    });

    test("GET /api/v3/project-summaries with category name in search", async () => {
      const res = await get(app, "/api/v3/project-summaries?search=Uncategorised");
      expect(res.statusCode).toBe(200);
      expect(res.body.length).toBeGreaterThan(0);
      expect(res.body.every((app: ProjectSummary) => app.categories?.includes("Uncategorised"))).toBe(true);
      expect(res.body.find((app: ProjectSummary) => app.categories?.includes("Uncategorised"))).toBeDefined();
    });

    test("GET /api/v3/project-summaries with search query filter searching for name", async () => {
      const res = await get(app, "/api/v3/project-summaries?search=oDecrafTE");
      expect(res.statusCode).toBe(200);
    ...
    test("GET /api/v3/project-summaries with search query filter searching for description", async () => {
      const res = await get(app, "/api/v3/project-summaries?search=" + encodeURIComponent("interesting things"));
      expect(res.statusCode).toBe(200);
      const result: ProjectSummary[] = res.body;
      expect(result.length).toBeGreaterThanOrEqual(1);
      expect(result.every((app: ProjectSummary) => app.description?.includes("interesting things"))).toBe(true);
    });

    test("GET /api/v3/project-summaries with device and category filters", async () => {
      const res = await get(app, "/api/v3/project-summaries?badge=troopers23&category=Silly");
      expect(res.statusCode).toBe(200);
      expect(res.body.length).toBeGreaterThan(0);
      expect(
        res.body.every(
          (app: ProjectSummary) =>
            app.badges?.includes("troopers23") && app.categories?.includes("Silly")
        )
      ).toBe(true);
    });

    test("GET /api/v3/project-summariesslugs=codecraft,codecrafter", async () => {
      const res = await get(app, "/api/v3/project-summaries?slugs=codecraft,codecrafter");
      expect(res.statusCode).toBe(200);
      expect(res.body.map((p: ProjectSummary) => p.slug)).toEqual(["codecraft", "codecrafter"]);
    });

    test("GET /api/v3/project-summaries?slugs=codecraft", async () => {
      const res = await get(app, "/api/v3/project-summaries?slugs=codecraft");
      expect(res.statusCode).toBe(200);
    ...
    test("GET /api/v3/projects/non-existent should return 404", async () => {
      const res = await get(app, "/api/v3/projects/non-existent");
      expect(res.statusCode).toBe(404);
    });

    test("GET /api/v3/projects/codecraft", async () => {
      const res = await get(app, "/api/v3/projects/codecraft");
      expect(res.statusCode).toBe(200);
      ...
      `);
      const sortedFiles = files.map((f) => f.sha256).sort().map((sha) => files.find((f) => f.sha256 === sha));
      expect(sortedFiles).toHaveLength(3);
      expect(sortedFiles.map((f) => f?.full_path)).toEqual(["__init__.py", "icon5.png", "metadata.json"]);
    ...
    test("GET /api/v3/projects/codecraft/rev1", async () => {
      const res = await get(app, "/api/v3/projects/codecraft/rev1");
      expect(res.statusCode).toBe(200);
      const project = res.body as ProjectDetails;
      const { version, ...restProject } = project;
      expect(restProject).toMatchInlineSnapshot(`
    ...
    test("GET /api/v3/projects/codecraft/rev2 (unpublished version)", async () => {
      const res = await get(app, "/api/v3/projects/codecraft/rev2");
      expect(res.statusCode).toBe(404);
    });
    ...
    test.each(["latest", "rev1"])(
      ...
      async (revision) => {
        const getRes = await get(app, `/api/v3/projects/codecraft/${revision}/files/metadata.json`);
    ...
    test("GET files using url prop should work same as from path", async () => {
      ...
      expect(project.version.published_at).toBeDefined();
      for (const file of files) {
        const requestFromFilePath = await get(app, `/api/v3/projects/codecraft/rev${project.version.revision}/files/${encodeURIComponent(file.full_path)}`);
        expect(requestFromFilePath.statusCode).toBe(200);
        const requestFromUrl = await get(app, new URL(file.url).pathname);
    ...
    test.each(["latest", "rev1"])(
      ...
      async (revision) => {
        const getRes = await get(app, `/api/v3/projects/codecraft/${revision}/files/__init__.py`);
        expect(getRes.statusCode).toBe(200);
        expect(getRes.text).toEqual("print('Hello world from the CodeCraft app')");
        expect(getRes.headers.get("content-disposition")).toEqual('attachment; filename="__init__.py"');
      }
    );
    ...
    describe("ping should return pong", () => {
      test.each([
        ...
        if (id) url += `?id=${id}`;
        if (mac) url += `&mac=${mac}`;
        const getRes = await get(app, url);
        expect(getRes.statusCode).toBe(200);
    ...
    describe("unpublished versions should not be requestable", () => {
      ...
      test.each(["rev0", "rev2", "rev3"])(
        "GET /projects/{slug}/%s",
        async (revision) => {
          const getRes = await get(app, `/api/v3/projects/codecraft/${revision}`);
          expect(getRes.statusCode).toBe(404);
        }
      );
    });
    ...
    describe("project-summaries fields query parameter", () => {
      test("GET /api/v3/project-latest-revisions", async () => {
        const getRes = await get(app, `/api/v3/project-latest-revisions`);
        expect(getRes.statusCode).toBe(200);
        ...
        expect(projectRevisionMap.find((p) => p.slug === "codecraft")).toMatchInlineSnapshot(`
          {
            "revision": 1,
            "slug": "codecraft",
          }
      ...
      test("GET /api/v3/project-latest-revisions?slugs=codecraft,codecrafter", async () => {
        const getRes = await get(app, `/api/v3/project-latest-revisions?slugs=codecraft,codecrafter`);
        expect(getRes.statusCode).toBe(200);
        const projectRevisionMap = getRes.body as ProjectLatestRevisions;
      ...
      test("GET /api/v3/project-latest-revisions?slugs=codecraft", async () => {
        const getRes = await get(app, `/api/v3/project-latest-revisions?slugs=codecraft`);
        expect(getRes.statusCode).toBe(200);
        const projectRevisionMap = getRes.body as ProjectLatestRevisions;
      ...
      test("GET /api/v3/project-latest-revisions/codecraft", async () => {
        const getRes = await get(app, `/api/v3/project-latest-revisions/codecraft`);
        expect(getRes.statusCode).toBe(200);
      ...
      test("GET /api/v3/stats", async () => {
        const getRes = await get(app, `/api/v3/stats`);
        expect(getRes.statusCode).toBe(200);

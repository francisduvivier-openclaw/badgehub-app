...
function handleApiRequest(path, url, { summaries, stats, badges, categories }) {
  ...
  if (path.startsWith("/projects/")) {
    ...
    if (!summary) {
      return json({ reason: `No project with slug '${slug}' found` }, 404);
    }
    return json({
      slug: summary.slug,
  ...
  if (path === "/badges") return json(badges);
...
self.addEventListener("fetch", (event) => {
  ...
  // loadPreviewData() waits for preview-data.json if the SW process was
  // restarted (module variables reset to null), so requests wait rather than
  // getting an immediate 503.
  event.respondWith(
    loadPreviewData()
      .then((data) => handleApiRequest(path, url, data))
      .catch((e) => {
